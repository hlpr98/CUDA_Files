...
//    image._imageChannels = 3;  
    ...
    if (fileInput.is_open()) {
        ...
        char widthStr[64], heightStr[64], numColorsStr[64], *p, channelStr[64]; 
        p = widthStr;                    
        if(isdigit(tmp)) {
	    ...
	    p = channelStr;
            ...
            *p = '\0';
            image._imageChannels = atoi(channelStr);
            std::cout << "Depth: " << image._imageChannels << std::endl;
            p = numColorsStr;
	//		fileInput.get();
            while(isdigit(*p = fileInput.get())) { 
                p++; 
...
void wbImage_save(wbImage_t& image, const char outputfile[]) {
    ...
    buffer[0] = ' ';
    outputFile.write(buffer, 1);
    sprintf(buffer,"%d", image._imageChannels);
    outputFile.write(buffer, strlen(buffer));
    buffer[0] = '\n';
    outputFile.write(buffer, 1);
...
//    std::cout<<solutionFile<<std::endl;
    ...
    if (image._imageChannels != solutionImage._imageChannels) {
       std::cout << "Depth is incorrect: expected " << solutionImage._imageChannels << " but got " << image._imageChannels << std::endl;
       exit(1);
    }		

    int channels = image._imageChannels;
