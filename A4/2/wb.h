...
unsigned int* wbImport(const char* fname, int* itemNum)
{
    ...
    unsigned int* fBuf = (unsigned int*) malloc(*itemNum * sizeof(unsigned int));
    if (!fBuf)
    {
        std::cout << "Unable to allocate memory for array of size " << *itemNum * sizeof(unsigned int) <<" bytes";
        exit(EXIT_FAILURE);
    }
	...
	fBuf[idx] = (unsigned int)(*sval.c_str());
	if(fBuf[idx]==0)
		std::cout<<"hi\n";
    }
    return fBuf;
...
unsigned int* wbImport_(const char* fname, int* itemNum)
{
...
{
    int solnItems;
    unsigned int *soln = (unsigned int*) wbImport_(wbArg_getInputFile(args, args.argc - 2), &solnItems);
